...
public class XLLog
{
    ...
    /**建议使用下面的接口*/  
    public static final String SD_FILE_PATH = "/SpeakInRecorder/log";	// log到文件时，文件的目录
    ...
    private static void initFileHandler() {
        if (mLogHandler == null) {
            synchronized (object) {
                if (mLogHandler == null) {
                    HandlerThread thread = new HandlerThread("XLLogThread");
                    thread.start();
                    mLogHandler = new LogHandler(thread.getLooper());
                }
            }
        }
    }
